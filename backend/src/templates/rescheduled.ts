...
export function bookingRescheduledTemplate(
  data: BookingEmailData & { oldDate?: string; oldTime?: string }
): { subject: string; html: string } {
  const subject = `📅 Booking Rescheduled — ${data.serviceName} on ${data.appointmentDate}`;

  const html = baseTemplate(`
    <div class="body">
      <span class="badge badge-gold" style="margin-bottom:20px;">Booking Rescheduled</span>
      <h1 class="h1">Your appointment has been <em>rescheduled.</em></h1>
      <p class="subtitle">Here are your updated booking details.</p>
      ...
      <div class="detail-card">
        ...
        <div class="detail-row">
          <span class="detail-label" style="color:#B91C1C;">Previous Date</span>
          <span class="detail-value" style="text-decoration:line-through;color:#9E968E;">${data.oldDate} at ${data.oldTime}</span>
        </div>
        <div class="detail-row">
          <span class="detail-label" style="color:#065F46;">New Date</span>
          <span class="detail-value" style="color:#065F46;font-weight:700;">${data.appointmentDate} at ${data.appointmentTime}</span>
        </div>
        ${data.staffName ? `
        <div class="detail-row">
      ...
      <center>
        <a href="${BRAND.website}/dashboard" class="btn">View Updated Booking</a>
      </center>
    </div>
  `, `Your ${data.serviceName} has been rescheduled to ${data.appointmentDate}`);
