...
// ── 3. Booking Cancelled ─────────────────────────────────────────────────────
export function bookingCancelledTemplate(data: BookingEmailData & { reason?: string }): { subject: string; html: string } {
  const subject = `❌ Booking Cancelled — ${data.serviceName}`;

  const html = baseTemplate(`
    <div class="body">
      <span class="badge badge-red" style="margin-bottom:20px;">Booking Cancelled</span>
      <h1 class="h1">Your booking has been <em>cancelled.</em></h1>
      <p class="subtitle">We're sorry to see you go, ${data.customerName.split(' ')[0]}.</p>
      ...
      <div class="detail-card">
        ...
        <tr class="detail-row">
          <td class="detail-label">Was scheduled for:</td>
      ...
      <p class="p">
        We'd love to have you back! Book a new appointment whenever you're ready.
      </p>

      <center>
        <a href="${BRAND.website}/booking" class="btn btn-gold">Book Again</a>
      </center>
    </div>
