...
// ── Template data type ───────────────────────────────────────────────────────
export interface ChangeRequestDeclinedEmailData {
  customerName: string;
  customerEmail: string;
  serviceName: string;
  appointmentDate: string; // "Friday, May 15, 2026" — the booking's CURRENT (unchanged) date
...
export function changeRequestDeclinedTemplate(
  data: ChangeRequestDeclinedEmailData
): { subject: string; html: string } {
  const actionWord = data.requestType === 'CANCEL' ? 'cancellation' : 'change';
  const subject = `Your ${actionWord} request wasn't approved — ${data.serviceName}`;

  const html = baseTemplate(`
    <div class="body">
      <span class="badge badge-gold" style="margin-bottom:20px;">Request Not Approved</span>
      <h1 class="h1">Your ${actionWord} request was <em>not approved.</em></h1>
      <p class="subtitle">
        Hi ${data.customerName.split(' ')[0]}, your original booking is still standing exactly as it was.
      </p>
      ...
      <p class="p">
        If you'd still like to make a change, you're welcome to submit a new request from your
        dashboard, or give us a call and we'll help you find something that works.
      </p>
  ...
  `, `Your ${actionWord} request for ${data.serviceName} was not approved`);
